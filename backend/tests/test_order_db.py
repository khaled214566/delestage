...
from app.models.enums import AllocationLevel, DeficitPlanStatus, OrderStatus, OrderType, UserRole
...
async def _create_test_user(db, role="DISPATCHER"):
    ...
    user = User(
        username=f"test_{role.lower()}_{datetime.now().timestamp()}",
        name=f"Test {role}",
...
async def test_create_order_from_validated_plan(adb):
    ...
    order = await order_service.create_order(adb, plan_id=plan.id, actor=user)
...
async def test_create_order_from_draft_plan_fails(adb):
    ...
    plan = DeficitPlan(
        date=date(2026, 9, 20),
        mode=OrderType.J_1,
        status=DeficitPlanStatus.DRAFT,
        created_by=user.id,
    )
    adb.add(plan)
    await adb.flush()
    ...
    with pytest.raises(ValueError, match="not VALIDATED"):
        await order_service.create_order(adb, plan_id=plan.id, actor=user)
...
async def test_run_allocation(adb):
    ...
    order = await order_service.run_allocation(adb, order_id=order.id, actor=user)
    assert order.status == OrderStatus.ALLOCATED
    nodes = (
        await adb.execute(
            select(AllocationNode).where(AllocationNode.order_id == order.id)
        )
    ).scalars().all()
    assert len(nodes) > 0
...
async def test_allocation_sum_conservation(adb):
    ...
    assert round(sum(c.target_mw for c in crc_nodes), 2) == round(national_nodes[0].target_mw, 2)
...
async def test_validate_order(adb):
    ...
    plan = await _create_validated_plan(adb, user)
    order = await order_service.create_order(adb, plan_id=plan.id, actor=user)
    await order_service.run_allocation(adb, order_id=order.id, actor=user)

    validated = await order_service.validate_order(adb, order_id=order.id, actor=user)
...
async def test_cancel_order(adb):
    ...
    cancelled = await order_service.cancel_order(adb, order_id=order.id, reason="Testing cancellation", actor=user)
...
async def test_invalid_transition(adb):
    ...
    with pytest.raises(ValueError, match="Cannot transition"):
        await order_service.validate_order(adb, order_id=order.id, actor=user)
